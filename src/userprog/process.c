...
process_execute (const char *file_name) 
{
  ...
  ***********************************************************/
  ...
  tid = thread_create (token, PRI_DEFAULT, start_process, &exec);
  if (tid != TID_ERROR){
  	sema_down(exec.loading_sema);
	  if(exec.load == true)
	    //add child to current thread's list of children
      //child_elem is retrieved from thread_create
      list_push_back(&thread_current()->children, &exec.child_elem);
	  else
	   return TID_ERROR;
  }
...
start_process (void *file_name_)
{
  ...
  /* If load failed, quit. */
  palloc_free_page (exec);
  if (!success) 
    thread_exit ();
  exec->child_elem = &thread_current()->child_elem;
  exec->load = success;
  sema_up(exec->loading_sema);
...
static bool setup_stack_helper(const char* cmd_line, uint8_t *kpage, uint8_t *upage, void **esp){
  ...
  char * ptr;
  char *save;
  //holds args for pushing to stack
  char** argv = malloc(sizeof(char*));
  //holds memory addresses of args pushed to stack
  char** addresses = malloc(sizeof(char*));
  //count of num of args
  int argc = 0;
  //count number of addresses
  int addc = 0;
  ptr = strtok_r(cmd_line, " ", &save);
  while(ptr != NULL)
  {
  ...
  //minus 1 since argc is total number of elements
  argc = argc - 1;
  int argc_temp = argc;
  //>= so it can run on 0th element
  while(argc >= 0){
    //should argv[argc] be &
    if(addresses[addc] = push(kpage, &ofs, argv[argc], sizeof(argv[argc]))==NULL){
      return false;
    }
    addc = addc + 1;
    argc = argc - 1;
  ...
  //need to start at beginning of argv since the first address
  //in addresses is the first to be pushed so another variable
  //is necessary
  int i = 0;
  //push a null onto the stack which would normally be 
  //the null that ends the array of address
  if(push(kpage, &ofs, &null, sizeof(null))==NULL){
    return false;
  }  
  //push memory addresses onto stack
  while(i < addc){
    //should addresses[addc] be &
    if(push(kpage, &ofs, addresses[addc], sizeof(addresses[addc]))==NULL){
      return false;
    }
    i = i + 1;
  }
  //push number of items pushed to stack onto the stack
  if(push(kpage, &ofs, argc_temp, sizeof(argc_temp))==NULL){
    return false;
  }
  //push null onto stack
  if(push(kpage, &ofs, &null, sizeof(null))==NULL){
