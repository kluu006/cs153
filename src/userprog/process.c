...
static bool setup_stack_helper(const char* cmd_line, uint8_t* kpage, uint8_t* upage, void ** esp)
{
	...
	char* ptr;
	strlcpy(ptr, cmd_line, strlen(cmd_line));
	char* token;
	char* save;
	...
	while(token != NULL){
		/*if(push(kpage, &ofs, token, strlen(cmd_line)) == NULL){
			return false;
		}*/
	...
	int new_argc = argc;
	while(argc > 0){
		if(mem[mem_count] = push(kpage, &ofs, &argv[argc], sizeof(argv[argc])) == NULL){
			return false;
		}	
	...
	while(mem_count > 0 ){
		if(push(kpage, &ofs, mem[mem_count], sizeof(mem[mem_count])) == NULL){
			return false;
		}
		mem_count = mem_count - 1;
	}
	if(push(kpage, &ofs, new_argc, sizeof(new_argc)) == NULL){
		return false;
	}
	if(push(kpage, &ofs, &null, sizeof null) == NULL){
		return false;
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
	...
	/* Set up stack. */
	if (!setup_stack (esp, file_name))//file_name = command line
		goto done;
...
setup_stack (void **esp, const char* cmd_line) 
{
	...
	{
		//make upage here
		//call setup_stack_helper
		success = install_page (((uint8_t *) PHYS_BASE) - PGSIZE, kpage, true);
		setup_stack_helper(cmd_line, kpage, ((uint8_t *) PHYS_BASE) - PGSIZE, esp);
		if (success){
			//*esp = PHYS_BASE;
			success = setup_stack_helper(cmd_line, kpage, ((uint8_t *) PHYS_BASE) - PGSIZE, esp);
		}
		else
			palloc_free_page (kpage);
	}
